...
## 2026-05-30 Jocheng 博客本机运行、内容整理和部署问题记录

本节记录 `E:\Work\8、Codex\Jocheng博客` 个人博客搭建、内容整理、搜索修复和 GitHub Pages 部署过程中已经确认的问题与解决方案。

### 本地运行和预览

Windows PowerShell 中优先使用 `npm.cmd`，避免被 `npm.ps1` 执行策略拦截：
...
如果 preview 进程 `Ctrl+C` 后没有退出，先检查端口：
...
`Get-CimInstance Win32_Process` 在当前环境可能返回拒绝访问；排查端口占用时优先用 `netstat -ano`。

### Pagefind 搜索索引

问题：搜索页提示“搜索索引尚未生成”或线上搜索资源 404。

原因：只运行 `npm run build` 会生成 Astro 页面，但不会生成 Pagefind 索引目录。
...
```powershell
Invoke-WebRequest -UseBasicParsing 'https://jocheng99.github.io/search/'
Invoke-WebRequest -UseBasicParsing 'https://jocheng99.github.io/pagefind/pagefind-ui.js'
```

当前搜索页通过动态插入脚本加载 Pagefind UI，路径跟随 `withBase("/pagefind/")`。GitHub Pages 用户主页仓库使用根路径 `/`，搜索页提示文案不应继续写 Gitee 的 `/coolcheng/` 子路径。

### 分类和标签中文路径

问题：首页“游戏记录入口”曾打开 404。

原因：分类和标签路径曾用 `encodeURIComponent` 直接生成中文 URL。GitHub Pages 对中文路径的解码行为会造成访问路径和构建目录不一致。

解决：`src/utils/taxonomy.ts` 使用稳定英文 slug 映射，例如：

```text
技术 -> /categories/tech/
游戏 -> /categories/game/
PPT 自动化 -> /tags/ppt-automation/
```

页面展示名称继续保留中文；只有 URL 使用英文 slug。新增中文分类或标签时，应同步补充稳定 slug，避免回退到不透明 hash 路径。

### GitHub Pages 部署

Gitee Pages 服务不可用后，当前部署目标改为 GitHub Pages 用户主页仓库：
...
推送源码后由 GitHub Actions 运行 `npm run build:search` 并发布 `dist/`。

### Git 提交和推送问题

问题：`git commit` 报 `Author identity unknown`。

原因：当前机器没有配置 Git 作者信息。
...
问题：`git push -u origin master --force-with-lease` 报 `stale info`。

原因：本地远程引用已过期，或者远程分支已有本地不知道的新状态。
...
确认远程旧站已备份、当前覆盖范围符合预期后，再重新执行推送。不要在未确认远程状态时盲目改用无保护的 `--force`。

当前本机未安装 GitHub CLI，`gh run list` 会提示 `gh` 不是可识别命令。部署完成检查可先用 `git push` 输出和线上 `Invoke-WebRequest` 抽查页面替代。

### 博客素材整理

`reference_materials/inbox/` 只作为未处理资料暂存区。资料转为博客后，应移动到：

```text
reference_materials/docs/blog_sources/<post-slug>/
```

如果素材是一组文件夹，保持文档、图片、PPTX 等原始结构整体移动。公开展示图片复制到：
...
generative-ai-prompt-assistant-guide
ppt-automation-drawing-workflow
```
